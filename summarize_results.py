...
def evaluate_file(filepath, model, gpus=1):
    ...
    for row, comet_score in zip(records, comet_scores):
        # Parse counts
        ...
        # Assign to bucket if term count 1-5
        term_count = ts + tf
        if 1 <= term_count <= 5:
...
def main(input_directory, output_directory,model_name="Unbabel/wmt20-comet-qe-da"):
    ...
    for fname in os.listdir(input_directory):
        ...
        # Write bucketed term averages CSV
        bucket_csv = os.path.join(output_directory, fname.replace('.csv','_bucketed.csv').replace('.tsv','_bucketed.csv'))
        with open(bucket_csv, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            header = ['terms', model_name]
            writer.writerow(header)
            for bucket_num in range(1, 6):
                writer.writerow([bucket_num, result_all['bucket_avgs'].get(bucket_num, 0)])
