...
def find_model_file(dest_dir, model_name):  # this one better
    if model_name:
        model_file = os.path.join(dest_dir,model_name)
    else:
...
class OpusState:
    def __init__(self, source_dir, model_name, eos_token_id=0):
        npz_path = find_model_file(source_dir, model_name)
        ...
        tokenizer_has_pad_token_id = (
            hasattr(self.tokenizer, "pad_token_id") and self.tokenizer.pad_token_id is not None
        )
        ...
        if cfg["tied-embeddings-src"]:
            if pad_added:
                ...
                self.wemb, self.final_bias = (self.state_dict["Wemb"], self.state_dict[BIAS_KEY])
                self.pad_token_id = pad_token_id
                # this just disables the check further down 
            ...
            # TODO: for the marian models with LLM vocabs, the pad token is already included,
            # find out why this causes the size mismatch
        ...
        # Process decoder.yml
        decoder_path = glob.glob(os.path.join(source_dir, '*decoder.yml'))[0]
        decoder_yml = cast_marian_config(load_yaml(decoder_path))
...
def convert(source_dir: Path, dest_dir, model_file_name):
    dest_dir = Path(dest_dir)
    dest_dir.mkdir(exist_ok=True)

    opus_state = OpusState(source_dir, model_file_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default=None, help="Name of the model file, used in cases with multiple models present in src folder.")
    ...
    convert(source_dir, dest_dir, args.model_name)
