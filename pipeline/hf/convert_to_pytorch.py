...
def find_model_file(dest_dir):  # this one better
    model_files = list(Path(dest_dir).glob("final.model.npz.best-perplexity.npz"))
...
def find_vocab_file(model_dir):
    return list(model_dir.glob("*vocab.spm"))[0]


def find_src_vocab_file(model_dir):
    return list(model_dir.glob("*source.spm"))[0]


def find_tgt_vocab_file(model_dir):
    return list(model_dir.glob("*target.spm"))[0]
...
class OpusState:
    def __init__(self, source_dir, eos_token_id=0):
        ...
        spm_path = find_vocab_file(self.source_dir)  # Locate vocab.spm file
        self.tokenizer = self.load_tokenizer_from_spm(spm_path)  # Load tokenizer from SentencePiece
        ...
        # Process decoder.yml
        decoder_yml = cast_marian_config(load_yaml(source_dir / "final.model.npz.best-perplexity.npz.decoder.yml"))
    ...
    def load_tokenizer_from_spm(self, spm_path: Path):
        sp = spm.SentencePieceProcessor()
        sp.Load(str(spm_path))
    
        # Create a vocabulary dictionary from the SentencePiece model
        vocab = {sp.id_to_piece(i): i for i in range(sp.get_piece_size())}
        # Add special tokens if not present
        special_tokens = ["<pad>"] # To avoid getting assertion error
        for token in special_tokens:
            if token not in vocab:
                vocab[token] = len(vocab)  # Assign a new ID for the token
        vocab_json_path=str(spm_path).replace(".spm",".json")
        with open(vocab_json_path, 'w') as f:
            json.dump(vocab, f, indent=2)
        
        # TODO: Fix for separate target and source vocabularies
        tokenizer = MarianTokenizer(source_spm=str(spm_path), target_spm=str(spm_path), vocab=vocab_json_path)
